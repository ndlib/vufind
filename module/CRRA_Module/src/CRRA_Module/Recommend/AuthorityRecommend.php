...
/**
	...
	//SEE Also Recommendation
        ...
        $requestForSeeAlso = new Parameters(
            array(
                ...
                'type1' => array('MainHeadingExactMatch'),
                'bool2' => array('NOT'),
                'lookfor2' => array($this->lookfor),
                'type2' => array('UseFor')
            )
        );
		...
		'BibCount' => $result->getBibCount()
            ...
            if (!$this->inArrayR($recordArray['heading'], $this->recommendationsForSeeAlso)) {
	...
	// UseFor Recommendation
		...
		'BibCount' => $result->getBibCount()
            ...
            // check for duplicates before adding record to recordSet
            if (!$this->inArrayR($recordArray['heading'], $this->recommendationsForUseFor)) {
